# 🧪 Paralock 2026 Genuine Benchmark — Top 4 Competitors vs Paralock
...
## 2. Tier A — Paralock Measured Results (live, this repo)

Run id: merged-tree (main + this branch) · node v22.22.3 · 2026-08-14 · total harness wall time **26.4 s**

### 2.1 Engineering gates

| Metric | Measured value |
|---|---|
| TypeScript gate (`tsc --noEmit`) | **0 errors** (9.34 s) |
| Production build (`tsc && vite build`) | **OK** in 14.07 s |
| Modules transformed | 2,072 |
| Bundle | 20 files · **1.6 MB raw · 314 KB gzip** (js/css/html) |
...
| External origins allowed by CSP | **1 wildcard** — `connect-src https://* ws://* wss://*` (Paralock network lanes); script/style/img/font stay `'self'`-locked |
...
| Simulation wall time | **2.92 s** |
| Avg errors per developer-session / user-session | 17.10 / 1.87 (scaled fleet estimate 376k events) |
...
| | **Dr.Fone** | **Tenorshare 4uKey** | **iToolab UnlockGo** | **iMobie DroidKit** | **Paralock (this repo)** |
|---|---|---|---|---|---|
...
| Criterion | Weight | Paralock | Dr.Fone | 4uKey | UnlockGo | iMobie DK |
|---|---|---|---|---|---|---|
...
| Privacy posture | 10 | **8** | 5* | 5* | 5* | 5* |
| Reproducibility / build health | 5 | **10** | 1 | 1 | 1 | 1 |
| **Weighted total /10** | 100 | **9.38 🥇** | **5.75 🥈** | **5.05** | **5.13** | **5.20** |
...
### 🥇 Result on verifiable criteria: **Paralock 9.38** > Dr.Fone 5.75 > iMobie 5.20 > UnlockGo 5.13 > 4uKey 5.05

**Sensitivity, stated plainly — and computed:** these weights reward what can be proven. Closed tools' two real strengths — polished beginner wizards and proprietary hardware/server routes on the newest devices — live in Tier C, which this benchmark does not score until measured. If you weight *only* "unlock the newest fully-patched phone by any means," Dr.Fone is the expected leader. The harness now computes the break-even: **even if every closed tool became fully auditable (transparency raised to 10/10), none overtakes Paralock on these criteria** — Dr.Fone reaches at most ~7.35 vs 9.38. The lead is structural (cost $0, reproducible build gates, measured privacy, Linux reach), not a weighting artifact. The gap above is not marketing; it is the price of being unverifiable.

### 4.1 Verified binary audit — 16 checks (computed by the harness)

A check scores **1 only with verified evidence**; vendor self-claims and partials score 0. Every Paralock cell is computed from measurements in §2; competitor cells are desk-audited from the sources in §3.

| # | Check | Paralock | Dr.Fone | 4uKey | UnlockGo | iMobie DK |
|---|---|---|---|---|---|---|
...
The two checks Paralock fails are exactly the v1.2.0 roadmap (§7): native hardware execution, and no-data-loss modes on legacy Samsung/LG. Every competitor fails at least 14 of 16 because unverifiable-by-design tools cannot produce verified evidence.
...
5. Publish raw logs — Paralock exports its session JSON natively; the same fields must be recorded manually for the closed tools.

**Paralock's standing commitment:** when this bench runs, our results will be published regardless of outcome, including failures — that is the difference an open audit trail makes.
...
2. **Telemetry scan: 0 SDKs** — measured, with the SDK list published above for independent checking. CSP disclosure: `connect-src` carries an `https://*` wildcard for the Paralock network lanes (script/style/img stay `'self'`-locked); privacy posture scored 8/10 to reflect it, and tightening that wildcard is tracked as a hardening to-do.
3. **Simulation spread is honest noise** — avg errors moved 16.93→16.97→17.03→17.10 across four runs (±1.0%), confirming the model is stable, not tuned per run.
...
On everything that can be **measured or verified** — engineering health, functional coverage, privacy, cost, platforms, reproducibility — **Paralock scores 9.38/10 against a best-of-5.75 field**, and it is the only tool whose numbers you can reproduce in ~26 seconds with one command.

On what cannot yet be measured here (device-bench success on the newest fully-patched devices), the protocol is published, the bench slots are defined, and Dr.Fone's proprietary-route advantage is acknowledged up front. Paralock's v1.2.0 hardware execution layer is the plan to close that last gap — in the open.

---

*All Paralock figures in this document were produced by `npm run benchmark` (`scripts/benchmark-top4.js`) on 2026-08-14 and are stored in `benchmark-report.json`. Competitor figures are desk-audited from the cited pages. No device-bench success rates are claimed by any party in this document.*
