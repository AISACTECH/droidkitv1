...
### ⚠️ Tauri schema law (verified 2026-08-13 — do NOT "fix" this backwards)

`bundle.publisher` is a **TOP-LEVEL** field of the Tauri 2.x config (docs.rs: tauri-utils
`BundleConfig.publisher: Option<String>`). `bundle.windows.publisher` **does not exist** — placing
it there fails `tauri build` with `bundle > windows: Additional properties are not allowed
('publisher' was unexpected)`. If any agent (or you) proposes moving `publisher` INTO
`bundle.windows`, that is the regression that broke `main` (commit 35b4404) and the two failed
publish runs — the correct placement is the bundle root, next to `copyright`. `npm run doctor`
now hard-fails on the wrong placement, and CI `cargo check --locked` catches it at compile time
(`tauri::generate_context!` validates the config).
...
5. **Only quote numbers that exist in a committed artifact.** The simulation numbers
   (20k users/10k devs: coverage 100.0%, success 65.8% → 84.0% Scenario B, ratings 4.15★/4.35★)
   come from `docs/SIMULATION-DEEP-COMPARISON-2026.md` — a synthetic model, label it as such.
   Per-category percentages like "FRP ≤13: 88 vs 95", "MiFi 82%", "button-phone 90%",
   "PC/laptop 92%", "black-screen 84%", "modem 78%" do **NOT exist in any generated benchmark** —
   never invent them. The measured per-category numbers live in the per-device tables of the two
   benchmark documents (e.g., FRP ≤14 segment raw: ours 75.6, TSM 76.0, 4uKey 66.5; network
   modem V1/V2 = 100 engine-verified; router = 80–85 vendor-documented; own-Wi-Fi = 70).
   When an older comparison doc (COMPARISON_2026_TOP_FRP_APPS.md) contains marketing-style
   phrasing, `docs/COMPARISON-2026-FINAL.md` supersedes it — quote the final.
